
package geometriksekilhesaplama;

import java.util.ArrayList;
import java.util.Scanner;


public class GeometrikSekilHesaplama {

    
    public static void main(String[] args) {
        
        Scanner scanner = new Scanner(System.in);
        
        String islemler = "İşlemler...\n"
                         +"1. Kare Alanı Hesapla\n"
                         +"2. Üçgen Alanı Hesapla\n"
                         +"3. Daire Alanı Hesapla\n"
                         +"Çıkış : q";
        
        while(true){
            System.out.println(islemler);
            System.out.println("Hnagi şeklin alanını hesaplamak istiyorsunuz : ");
            String sekil_turu = scanner.nextLine();
            
            if(sekil_turu.equals("q")){
                System.out.println("Programdan çıkılıyor...");
                break;
            }
            else if(sekil_turu.equals("1")){
                System.out.println("Karenin keanrı :");
                int kenar = scanner.nextInt();
                scanner.nextLine();
                
                Sekil sekil = new Kare("Kare1" , kenar);
                
                sekil.alanHesapla();
            }
            else if(sekil_turu.equals("2")){
                System.out.print("Kenar 1 : ");
                int kenar1 = scanner.nextInt();
                System.out.print("Kenar 2 : ");
                int kenar2 = scanner.nextInt();
                System.out.print("Kenar 3 : ");
                int kenar3 = scanner.nextInt();
                scanner.nextLine();
                
                Sekil sekil = new Ucgen("Uncgen 1", kenar1, kenar2, kenar3);
                
                sekil.alanHesapla();
                
            }
            else if (sekil_turu.equals("3")){
                System.out.print("Dairenin Yarıçapı : ");
                int yaricap = scanner.nextInt();
                scanner.nextLine();
                Sekil sekil =new Daire("Daire 1", yaricap);
                
                sekil.alanHesapla();
            }
            else{
                System.out.println("Geçersiz işlem...");
            }
        }
       
    }
    
}
